...
export type AndroidFilesystemMethod =
	| 'mkdir'
	| 'readdir'
	| 'readFile'
	| 'writeFile'
...
export function androidFilesystem<T = unknown>(
...
export function androidOpenFile(
	filePath: string,
	contentType: string,
): Promise<void> {
	if (!shouldUseParentPostMessageBridge()) {
	...
	return new Promise<void>((resolve, reject) => {
		...
		window.addEventListener('message', handleResponse);
		window.top?.postMessage(
			{ type: 'CCS_ANDROID_FILE_OPEN_REQUEST', id, filePath, contentType },
			'*',
		);
	});
}
...
export function androidCaptureViaBridge(): Promise<{
	...
	return new Promise((resolve, reject) => {
		...
		const timeout = window.setTimeout(() => {
			window.removeEventListener('message', handleResponse);
			reject(new Error('Android 相机操作超时'));
		}, 120000);

		function handleResponse(event: MessageEvent<AndroidCameraResponse>) {
			if (
				event.data?.type !== 'CCS_ANDROID_CAMERA_RESPONSE'
				|| event.data.id !== id
			)
				return;
			window.clearTimeout(timeout);
...
export async function readAndroidText(path: string): Promise<string> {
	...
	>('readFile', { path, directory: Directory.Data, encoding: Encoding.UTF8 });
	return String(result.data);
}

export async function writeAndroidText(
	path: string,
	text: string,
): Promise<void> {
...
export async function readAndroidBinary(path: string): Promise<Uint8Array> {
	const result = await androidFilesystem<
		Awaited<ReturnType<typeof Filesystem.readFile>>
	>('readFile', { path, directory: Directory.Data });
	return base64ToBytes(String(result.data));
}

export async function writeAndroidBinary(
	path: string,
	bytes: Uint8Array,
): Promise<void> {
	await androidFilesystem('writeFile', {
...
export async function getAndroidDirectoryBytes(path: string): Promise<number> {
	try {
		...
		for (const entry of entries.files) {
			...
			if (type === 'directory')
				total += await getAndroidDirectoryBytes(childPath);
...
export async function getAndroidFileUri(path: string): Promise<string> {
	...
	>('getUri', { path, directory: Directory.Data });
	return result.uri;
}
...
export async function getOpfsSubDir(
...
export async function listOpfsFileNames(
	directory: FileSystemDirectoryHandle,
): Promise<string[]> {
	const names: string[] = [];
	for await (const [name, handle] of (
		directory as DirectoryWithEntries
	).entries()) {
...
export async function getOpfsDirectoryBytes(
	directory: FileSystemDirectoryHandle,
): Promise<number> {
	let total = 0;
