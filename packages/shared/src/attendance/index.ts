...
export function createAttendanceService(deps: AttendanceDependencies = {}) {
	...
	async function locate(): Promise<LocationFix> {
		...
		return locateWithBrowser(
			runtime.kind === 'electron' ? ELECTRON_LOCATION_TIMEOUT : BROWSER_LOCATION_TIMEOUT,
		);
	}

	async function locateWithCapacitor(): Promise<LocationFix> {
		if (!Geolocation) throw new Error('Capacitor Geolocation 插件未注入');
	 ...
	 */
	async function punch(type: PunchType, currentRecord?: AttendanceRecord): Promise<{
	...
	function buildMapLink(location: LocationFix): MapLink {
		...
		const label =
			runtime.kind === 'web' ? '在 Bing 地图中查看' : '在浏览器中查看地图';
	...
	function normalizeLocationError(error: unknown): string {
		if (typeof error === 'object' && error && 'code' in error) {
			...
			if (code === 1)
				return '定位权限被拒绝，请在系统或浏览器设置中允许位置权限';
			if (code === 2)
				return '暂时无法获取位置，请确认网络、GPS 或系统位置服务已开启';
