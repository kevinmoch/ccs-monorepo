 ...
 */
export function createDocumentCatalog(
	localDocuments: OfflineDocument[],
	manifestUrl?: string,
) {
	/**
	 * 加载文档目录（在线时合并远程清单）。
...
function mergeCatalogs(
	localCatalog: OfflineDocument[],
	remoteCatalog: OfflineDocument[],
): OfflineDocument[] {
	const merged = new Map(
		localCatalog.map((document) => [document.id, document]),
	);
