...
export async function checkDocumentUpdate(document: OfflineDocument): Promise<CachedDocumentMeta | undefined> {
  const electron = getElectronOfflineDocs<ElectronOfflineDocsBridge>();
  if (electron) return electron.checkDocumentUpdate(document, resolveOfflineDocsUrl(document.url));
...
export async function downloadDocumentToOpfs(
  ...
  if (!isOpfsAvailable()) throw new Error(i18next.t('offlineDocs.errorOpfsUnavailable'));
  ...
  try {
    ...
    if (!response.ok && response.status !== 206) throw new Error(i18next.t('offlineDocs.errorDownloadFailed', { status: response.status }));
    validateDocumentResponse(response, document);
    if (!response.body) throw new Error(i18next.t('offlineDocs.errorNoReadableStream'));
    ...
    onProgress(buildProgress(document.id, receivedBytes, totalBytes, canResume, canResume ? i18next.t('offlineDocs.resumingDownload') : i18next.t('offlineDocs.downloadingProgress')));
    ...
    onProgress(buildProgress(document.id, receivedBytes, totalBytes ?? receivedBytes, canResume, i18next.t('offlineDocs.downloadedOffline')));
    return completed;
  } catch (error) {
    if (writable) {
      try {
...
function getLocalFileName(document: OfflineDocument) {
  const extension = getFileExtension(document.url) || document.fileType || deriveFileType(document.mimeType);
...
function validateDocumentResponse(response: Response, document: OfflineDocument) {
  ...
  if (contentType.includes('text/html') && !document.mimeType.toLowerCase().includes('html')) {
    throw new Error(i18next.t('offlineDocs.errorHtmlResponse'));
  }
}
