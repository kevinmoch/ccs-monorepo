  ...
  <section class="offline-doc-page">
      ...
      <div>
        ...
        <div class="title-row">
          ...
          <div class="docs-server-input">
            <input id="docs-server-url" v-model="docsBaseUrl" type="text" @change="setOfflineDocsBaseUrl(docsBaseUrl)" />
          </div>
        </div>
    ...
    <div v-else class="doc-workspace">
      <aside class="doc-list-panel">
        ...
        <div class="doc-list">
          <article v-for="row in filteredRows" :key="row.document.id" class="doc-row" :class="[{ active: row.document.id === selectedId }, `status-${row.status}`]">
            ...
            <div class="progress-area" :class="{ visible: row.progress }" :aria-hidden="!row.progress">
              ...
              <small v-if="row.progress"
                >{{ row.progress.percent ?? 0 }}% · {{ formatBytes(row.progress.receivedBytes) }} / {{ row.progress.totalBytes ? formatBytes(row.progress.totalBytes) : '未知大小' }} ·
                {{ row.progress.message ?? (row.progress.resumable ? '续传中' : '下载中') }}</small
              >
    ...
    <section class="cache-panel">
      ...
      <div class="cache-stats">
        <div>
          <span>缓存文件</span><strong>{{ formatBytes(stats.cachedBytes) }}</strong>
        </div>
        <div>
          <span>中断文件</span><strong>{{ formatBytes(stats.partialBytes) }}</strong>
        </div>
        <div>
          <span>元数据</span><strong>{{ formatBytes(stats.metadataBytes) }}</strong>
        </div>
        <div>
          <span>已选择</span><strong>{{ formatBytes(totalSelectedBytes) }}</strong>
        </div>
      </div>
...
.doc-hero {
  ...
  gap: 18px;
  padding: 22px;
  background: linear-gradient(135deg, rgba(20, 184, 166, 0.16), rgba(37, 99, 235, 0.1) 52%, rgba(245, 158, 11, 0.12)), #ffffff;
}
