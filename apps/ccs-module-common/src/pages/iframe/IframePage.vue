...
const computedCards = computed(() => {
  ...
  // CCS_BASE_URL may be overridden at runtime with a tenant-qualified prefix (e.g.
  // `/CCS_Tenant_Gray`) once ccs-framework resolves it after login, shared via same-origin
  // sessionStorage — scoped to the current tab/app session only, never persisted long-term.
