      ...
      <div class="portal-main">
        ...
        <template v-for="(section, idx) in allSections" :key="section.id">
          ...
          <template v-if="idx < toolCardCount">
            <h2
              class="portal-section-title"
              :class="{ 'portal-card__header--link': section.title.url && !section.title.disabled }"
              :title="pickTitle(section.title, language)"
              @click="section.title.url && !section.title.disabled && handleNavigate(section.title)"
            >
              {{ pickTitle(section.title, language) }}
            </h2>
