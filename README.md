...
| 通用模块   | `apps/ccs-module-common` | Vue 3 + TypeScript + Tailwind CSS    | 生产级共享模块，提供 CCS 系统页面 iframe 嵌入及 25 个专业门户导航页                    |
...
| 命令              | 说明                                                                                    |
| ----------------- | --------------------------------------------------------------------------------------- |
| `pnpm preview`    | 预览 `dist/web/` 构建产物（默认 HTTPS，端口 3000；`CCS_PREVIEW_SSL=false` 可切换 HTTP） |
...
| 页面       | 路由                | 说明                   |
| ---------- | ------------------- | ---------------------- |
| 我的工作台 | `/my-workspace`     | CCS 工作台 Portal 首页 |
| 我的项目   | `/my-projects`      | 项目列表               |
| 我的待办   | `/my-todos`         | 待办任务列表           |
| 严重超期   | `/severely-overdue` | 严重超期效能台账       |
| 超期       | `/overdue`          | 超期效能台账           |
| 通知公告   | `/notifications`    | 系统通知公告           |
| 系统设置   | `/settings`         | 系统管理配置           |
| IT 支持    | `/it-support`       | 问题提报表单           |
| 帮助指南   | `/help-guide`       | 用户指南模板库         |
...
| 门户         | 路由                  | 卡片类型                            |
| ------------ | --------------------- | ----------------------------------- |
| 投资立项管理 | `/portal-investment`  | PortalInvestmentCard（含 Tab 切换） |
| 工程管理     | `/portal-engineering` | PortalMenuCard                      |
| 采购管理     | `/portal-procurement` | PortalProcurementCard（三分区布局） |
| 验收管理     | `/portal-acceptance`  | PortalMenuCard                      |
| EHS 管理     | `/portal-ehs`         | PortalMenuCard                      |
| 成本管理     | `/portal-cost`        | PortalMenuCard                      |

**L2 二级专业（10 个）：**

| 门户     | 路由                         |
| -------- | ---------------------------- |
| 建筑设计 | `/portal-architecture`       |
| 土建工程 | `/portal-civil-engineering`  |
| 幕墙工程 | `/portal-curtain-wall`       |
| 暖通工程 | `/portal-hvac`               |
| 强电工程 | `/portal-power-electrical`   |
| 弱电工程 | `/portal-weak-current`       |
| 精装工程 | `/portal-interior-fit-out`   |
| 软装工程 | `/portal-soft-furnishing`    |
| 景观工程 | `/portal-landscape`          |
| 工艺工程 | `/portal-process-mechanical` |
...
main.ts 启动
  ...
  └── mount Vue App
        └── Router → 各 Page
              └── CardGrid → config.ts 定义的卡片列表
                    ├── IframeCard → <iframe src="...">（全屏嵌入）
                    ├── PortalMenuCard → 从 menuData 渲染菜单网格
                    ├── PortalInvestmentCard → Tab + 工具库/看板
                    └── PortalProcurementCard → 能力→作业→运营 三分区
```
