...
- [14. CCS 登录与认证门禁](#14-ccs-登录与认证门禁)
  ...
  - [14.5 租户 URL 解析与会话级覆盖](#145-租户-url-解析与会话级覆盖)
  - [14.6 Cookie 同步与认证请求](#146-cookie-同步与认证请求)
  - [14.7 fetchProxy：全局同源认证请求代理](#147-fetchproxy全局同源认证请求代理)
  - [14.8 开发模式与生产模式](#148-开发模式与生产模式)
- [15. AI 智能助手与 WebSkill 技能管理](#15-ai-智能助手与-webskill-技能管理)
  - [15.1 概述](#151-概述)
  - [15.2 AI 智能助手对话框](#152-ai-智能助手对话框)
  - [15.3 OpenUI：生成式 UI 渲染引擎](#153-openui生成式-ui-渲染引擎)
  - [15.4 演示场景引擎](#154-演示场景引擎)
  - [15.5 LLM 大模型集成](#155-llm-大模型集成)
  - [15.6 运行观测与链路追踪](#156-运行观测与链路追踪)
  - [15.7 WebSkill 智能技能管理器](#157-webskill-智能技能管理器)
  - [15.8 技能元数据核心库](#158-技能元数据核心库)
...
| 组成部分   | 路径                     | 技术栈                               | 说明                                                                                                                                      |
| ---------- | ------------------------ | ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------- |
| 框架主应用 | `apps/ccs-framework`     | React 19 + TypeScript + Tailwind CSS | Portal 壳（⚠️ **AI 生成，禁止手动修改**），提供全局导航、主题切换、多语言、iframe 调度、CCS 认证门禁、**AI 智能助手 & WebSkill 技能管理** |
| 通用模块   | `apps/ccs-module-common` | Vue 3 + TypeScript + Tailwind CSS    | 生产级共享模块，提供统一 iframe 嵌入入口及 19 个专业门户导航页                                                                            |
...
- **认证门禁**（Auth Gate）：框架启动时自动检测 CCS 系统的可达性与登录状态，支持 Electron（原生 BrowserWindow）、Android（原生 Activity）和 Web（整页重定向到 CCS 登录入口）三种登录方式。详见 [14. CCS 登录与认证门禁](#14-ccs-登录与认证门禁)
...
# CCS_TENANT_URL=https://ccs.huaweicloud.com
# CCS_TENANT_LOGIN=https://ccs.huaweicloud.com/ierp/index/login.html
```

| 环境变量           | 说明                                                    | 必填              |
| ------------------ | ------------------------------------------------------- | ----------------- |
| `CCS_BASE_URL`     | CCS 主系统的基础 URL（Web 端默认使用）                  | 生产环境必填      |
| `CCS_LOGIN_URL`    | CCS 主系统的登录页 URL（Web 端默认使用）                | 生产环境必填      |
| `CCS_TENANT_URL`   | CCS 租户系统的基础 URL（Electron / Android 默认使用）   | 生产环境必填      |
| `CCS_TENANT_LOGIN` | CCS 租户系统的登录页 URL（Electron / Android 默认使用） | 生产环境必填      |
| `ANDROID_HOME`     | Android SDK 路径                                        | 构建 Android 时需 |
| `JAVA_HOME`        | JDK 路径（需 JDK 21）                                   | 构建 Android 时需 |

> **说明**：这些环境变量在开发模式（`pnpm dev`）下不需要设置——开发模式下认证门禁自动跳过。构建生产包（`pnpm build:web` / `build:electron` / `build:android`）时必须全部配置。
>
> **平台默认系统选择**：Electron 和 Android 原生端**默认使用 `CCS_TENANT_URL` / `CCS_TENANT_LOGIN`**（租户系统），Web 端**默认使用 `CCS_BASE_URL` / `CCS_LOGIN_URL`**（主系统，其登录 UI 可通过下拉框手动切换到租户系统）。
...
| `pnpm ccs:ssl`     | 启动本地 mock CCS 服务器（HTTPS，端口 63735），提供 `/ierp` 登录页与 `/kapi` 模拟接口，用于联调认证门禁与 fetchProxy |
...
`CardDefinition` 支持两种写法：**扁平写法**和**嵌套写法**（`layout` 包裹），项目中均有使用（`ccs-module-common` 的门户页用扁平写法，`ccs-module-test` 的页面用嵌套写法）。`CardGrid` 内部会优先取 `layout.colSpan`，fallback 到顶层 `colSpan`。

```typescript
// src/pages/user/config.ts — 扁平写法示例
...
```typescript
// apps/ccs-module-test/src/main.ts（简化示意；实际文件还包含 URL 参数初始化、
// pending 主题/语言预缓存和错误兜底逻辑）
...
// 响应格式（error 为可选字段，框架处理失败时携带；收到 error 或不在 iframe 中时 Promise 会 reject）
{ type: 'CCS_STORE_RESPONSE', reqId: 'req_xxx', key: 'project', value: { ... }, error?: string }
...
`apps/ccs-framework/src/lib/store-broker.ts` 中的 `useStoreBroker()` hook 实现了 Store Broker（由 `App.tsx` 挂载），职责如下：
...
| 框架预置 key       | `'selectedBillCode'`          | 框架当前选中的单据编号                      |
| 框架预置 key       | `'menuData'`                  | 框架完整菜单树（门户页据此渲染导航卡片）    |
...
> ⚠️ **提醒**：`apps/ccs-framework` 的代码由 Google AI Studio 生成，**请勿手动修改该工程的任何文件**。新增菜单项需要通过 Google AI Studio 完成——将菜单结构描述给 AI，由其生成 `menu1~13.ts` 文件后导出替换。手动添加的菜单项将在下一次 AI 导出时被覆盖。

> ℹ️ **当前菜单方案的说明**：
>
> 当前菜单内容定义在 `apps/ccs-framework/src/lib/menu1~13.ts` 中，是**静态 TypeScript 文件**（另有 `menu21.ts`/`menu22.ts` 两个被 `menu2.ts` 拆分引用的部分文件）。这是为了方便本地开发调试——无需依赖后端服务即可完整运行。`menu_data.ts` 中的 `GET_MENU_DATA()` 函数直接返回这些静态数据。
>
...
创建子模块后，需要在 `apps/ccs-framework/src/lib/menu1~13.ts` 中新增菜单项，才能在框架侧边栏中看到入口。

### 5.1 添加菜单项

> **操作方式**：在 Google AI Studio 中向 AI 描述需要新增的菜单结构，AI 会生成对应的 `menu1~13.ts` 代码。导出时选择全量替换即可。
...
- **CCS 系统页面（外部 iframe）**：`ccs-module-common` 的 `IframePage` 使用 `CCS_BASE_URL` 拼接相对路径。登录后框架通过 `sessionStorage` 写入 `ccs-base-url-override`（含租户前缀的完整基础 URL，仅当前会话有效），`IframePage` 优先使用此覆盖值，确保 iframe 始终指向正确的租户限定地址（详见 [14.5 租户 URL 解析与会话级覆盖](#145-租户-url-解析与会话级覆盖)）
...
菜单图标使用 [Lucide React](https://lucide.dev/icons/) 图标库，在 `menu1~13.ts` 中 import 需要的图标即可。
...
子模块在 `main.ts` 中通过本地函数 `getIframeRoute()` 解析（`readIframeProps()` 只处理查询参数，hash 兼容逻辑在 `getIframeRoute()` 中）：
...
#### 5.4.5 `__ccs_language` 和 `__ccs_theme` 参数
...
对于 `https://` 开头的菜单 URL（如 系统链接），框架直接设置 iframe `src` 为完整 URL，不经过模块路由解析。外部 iframe 同样接收 `SYNC_STATE` 消息（主题/语言），但不处理路由导航。
...
此机制确保嵌入的 CCS 系统页面中的所有链接都不会弹出新窗口，保持统一的宿主壳体验。
...
├── safety-handbook2.png     # 示例文档：安全文明施工图示（第二页）
...
| `fileType`     | string  | ❌   | 文件类型标识；缺省时从 `mimeType` 自动推导              |
...
`checkDocumentUpdate()` 函数（`@ccs/shared/offline-docs`）通过 HTTP `HEAD` 请求获取服务器上文件的元信息，与本地缓存的元数据对比。上述代码片段为 Web 分支的简化示意；实际实现会优先走 Electron IPC、其次 Android 原生检测，Web 为兜底分支，且当响应头缺失时会回退到清单中的 `document.etag` / `lastModified` / `size` 字段：
...
项目的本地 HTTPS 涉及两套相互独立的自签名证书：

- **`@vitejs/plugin-basic-ssl` 自动证书**：`pnpm dev:ssl`、`pnpm dev:ssl-all`、`pnpm preview:ssl` 使用，由插件自动生成并维护在 `node_modules/.vite/basic-ssl/`，无需人工干预
- **`documents/cert.pem` / `documents/key.pem`**：由开发者自行准备的证书（可用 mkcert 或 openssl 生成），仅供以下三个 Node 服务使用
...
| `pnpm ccs:ssl`    | mock CCS 服务器（端口 63735）的 HTTPS |

使用 basic-ssl 自动证书的服务：

| 服务               | 使用场景                            |
| ------------------ | ----------------------------------- |
| `pnpm dev:ssl`     | 框架开发服务器（端口 3000）的 HTTPS |
| `pnpm dev:ssl-all` | 框架 + 所有子模块开发服务器的 HTTPS |
| `pnpm preview:ssl` | 构建预览服务器（端口 3000）的 HTTPS |
...
`documents/cert.pem` 和 `documents/key.pem` 不会被任何命令自动生成——删除后 `pnpm doc:ssl` / `pnpm upload:ssl` / `pnpm ccs:ssl` 会因找不到文件而失败。需要重新生成时，请自行使用 mkcert 或 openssl 生成，例如：
...
basic-ssl 的自动证书如需重新生成，删除 `node_modules/.vite/basic-ssl/` 目录后重启对应服务即可。
...
`apps/ccs-module-test` 是一个演示子模块，包含四个完整的功能页面（另有一个独立访问时使用的 `/` 首页），展示了离线优先（Offline-First）的工程现场场景。
...
- **功能演示**：
  - 调用摄像头拍照
  - 照片本地缓存（Web 端 OPFS；Android 应用私有目录 / Electron userData）
  - 离线照片队列上传（网络恢复后自动上传）
  - 上传进度追踪
...
| **页面数**   | 20 个页面（1 个 iframe 统一入口 + 19 个专业门户） | 4 个页面             |
...
所有 CCS 系统的 iframe 嵌入页面**统一收敛到单一路由** `/iframe`，URL 由菜单数据通过 **查询参数** `?url=...&top=...` 动态传入：
...
使用 `PortalMenuCard` / `PortalInvestmentCard` / `PortalSubMenuCard`，从框架通过 `globalStore.get('menuData')` 拉取菜单树数据，渲染为可点击的门户导航卡片。
...
| 规划设计管理 | `/portal-planning-design`   | PortalMenuCard       | `L1-2`      | 分类卡片 + seperateLine 工具组     |
| 工程管理     | `/portal-engineering`       | PortalMenuCard       | `L1-3`      | 分类卡片 + seperateLine 工具组     |
| 采购管理     | `/portal-procurement`       | PortalSubMenuCard    | `L1-4`      | 三分区：能力平台→作业平台→运营平台 |
...
| 资产库       | `/portal-asset-library`     | PortalSubMenuCard    | `L1-8`      | 分区式导航（同采购管理）           |
| 运营中心     | `/portal-operations-center` | PortalSubMenuCard    | `L1-9`      | 分区式导航（同采购管理）           |
        ...
        └── Router（iframe 内自动切换 hash 模式）
              └── 各 Page → onMounted 中 globalStore.get('menuData')
                    └── CardGrid → config.ts 定义的卡片列表（传入 rootId / menuData props）
                          ...
                          └── PortalSubMenuCard → 分区式门户（L1-4 采购 / L1-8 资产库 / L1-9 运营中心）：seperateLine 分区，tool-card 与 category-card 混排
```
...
- **rootId 查找机制**：通过 `findMenuNode(menuData, rootId)` 在完整的菜单树中定位节点——每个 Portal 页面只需在 `config.ts` 中指定 `rootId`，不需要重复维护菜单数据（例外：投资立项页的 `L1-1` 硬编码在 `PortalInvestmentCard` 组件内）
...
- **CCS_BASE_URL 运行时覆盖**：`IframePage.vue` 优先读取 `sessionStorage` 中的 `ccs-base-url-override`（登录后由框架写入的租户限定 URL，仅当前会话有效），fallback 到构建时的 `CCS_BASE_URL`
...
| **PortalSubMenuCard**    | 采购管理 / 资产库 / 运营中心（3 个门户） | 按 `seperateLine` 切分为多个分区并渲染居中的分区标题：工具类分区用 tool-card 样式，业务分区用 category-card 样式纵向排列 |
...
4. 在首页「我的应用」卡片中点击「材料检查」图标进入离线文档页面
...
3. 在浏览器中打开 `https://localhost:3000`
4. 在首页「我的应用」卡片中点击「随手拍」图标进入离线拍照页面
...
1. 启动 `pnpm dev`
2. 在首页「我的应用」卡片中点击「移动考勤」图标进入考勤打卡页面
...
| `ccs-framework/package-web.json`   | `electron`, `electron-builder`, `electron-vite`, Capacitor 全家桶                                                        | 仅保留 Web 运行时所需依赖（React、Tailwind、Vite、motion、undici 等）                                             |
...
- 文档下载到用户数据目录（`app.getPath('userData')/ccs-offline-docs-v1/`），而非 OPFS
- 支持断点续传和更新检测
...
| 工具            | 版本要求                                                   | 下载地址                                                                                                                        |
| --------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| **Android SDK** | API 36（`compileSdkVersion` / `targetSdkVersion` 均为 36） | [Android Studio](https://developer.android.com/studio) 或 [命令行工具](https://developer.android.com/studio#command-line-tools) |
...
| `CCS_ANDROID_KEYSTORE_PASSWORD` | 密钥库密码                             | —                       |
| `CCS_ANDROID_KEY_PASSWORD`      | 密钥密码                               | —                       |
| `CCS_ANDROID_PREPARE_ONLY`      | 设为 `1` 时仅同步 Web 资源、不构建 APK | —                       |

> 以上 `CCS_ANDROID_*` 变量均支持不带前缀的 `ANDROID_*` 别名（如 `ANDROID_APPLICATION_ID`），两者同时存在时优先取 `CCS_ANDROID_*`。
...
- `manifest.json`（本次构建的元信息，如应用 ID、版本、产物清单）
...
```bash
# 命令行参数
pnpm build:android --prepare-only
...
| 上传服务（HTTP）                  | 8082                                           |
| 上传服务（HTTPS）                 | 8083                                           |
| mock CCS 服务器（`pnpm ccs:ssl`） | 63735                                          |
...
项目的本地 HTTPS 使用两套相互独立的证书：

- **dev / preview 的 HTTPS**：`pnpm dev:ssl`、`pnpm dev:ssl-all`、`pnpm preview:ssl` 使用 `@vitejs/plugin-basic-ssl` 自动生成的证书（存放在 `node_modules/.vite/basic-ssl/`，由插件自动维护，无需人工干预）
- **doc / upload / mock CCS 服务的 HTTPS**：`pnpm doc:ssl`、`pnpm upload:ssl`、`pnpm ccs:ssl` 使用 `documents/cert.pem` 和 `documents/key.pem`（需开发者自行准备，可用 mkcert 或 openssl 生成）
...
- SW 注册在 `apps/ccs-framework/index.html` 的内联 `<script>` 中
...
| PWA 不显示安装按钮         | 确保使用 `pnpm preview:ssl`（HTTPS）访问                                                                                               |
...
| 生产构建后提示无法进入     | 确认 `.env` 中已配置 `CCS_BASE_URL`、`CCS_LOGIN_URL`、`CCS_TENANT_URL`、`CCS_TENANT_LOGIN` 四个变量                                    |
| Electron 中登录窗口白屏    | 检查 `CCS_TENANT_LOGIN` 是否正确（Electron 默认使用租户系统登录页）；确认主进程未报证书错误                                            |
| Android 登录后仍无法进入   | 确认使用 HTTP 文档/上传服务（Android 不信任自签名 HTTPS）；检查 `AndroidManifest.xml` 中网络权限                                       |
| Web 端一直停在检查界面     | Web 生产模式下认证门禁通过整页重定向完成登录；确认 `.env` 中的 `CCS_BASE_URL` / `CCS_TENANT_LOGIN` 正确，且 CCS 系统的回跳地址配置无误 |
...
`ccs-framework` 启动时内置了 **CCS 系统认证门禁**（Auth Gate），应用于以下场景：
...
门禁在框架渲染任何页面之前执行。**离线状态下始终放行**（仅通过设备级网络检测，不请求任何 CCS URL）；在线时则根据平台能力决定是否需要用户完成登录。
...
stateDiagram-v2
    [*] --> checking : 应用启动
    checking --> ready : 离线（设备级检测，不请求任何 CCS URL）
    checking --> ready : 在线 + 已登录（原生平台可检测）
    checking --> login_required : 在线 + 未登录

    login_required --> ready : 登录成功（原生自动检测）
    login_required --> login_failed : 登录取消 / 失败（仅原生平台）
    login_failed --> login_required : 重试
    login_failed --> ready : 点击「进入系统」跳过登录
    login_failed --> [*] : 退出应用
```

| 状态             | 含义                                                     | UI 表现                                                                 |
| ---------------- | -------------------------------------------------------- | ----------------------------------------------------------------------- |
| `checking`       | 正在检测设备网络状态（原生平台会进一步验证登录态）       | 全屏 Loading 动画                                                       |
| `ready`          | 认证通过，正常进入应用                                   | 无遮罩（正常渲染主界面）                                                |
| `login-required` | 在线但未登录，需要用户完成登录（仅原生平台会到达此状态） | Loading 遮罩 + 自动弹出的原生登录窗口（见 [14.3](#143-各平台实现差异)） |
| `login-failed`   | 原生登录窗口被关闭但未检测到登录成功                     | 全屏错误提示 + 重试 / 进入系统 / 退出 三个按钮                          |

### 14.3 各平台实现差异

#### Electron（原生登录窗口）

- **网络检测**：主进程使用 `net.isOnline()` 快速判断设备是否离线，离线直接放行
- **登录态检测**：在线时使用 `net.request`（跟随重定向，携带真实 Session Cookie）验证是否已登录 `CCS_TENANT_LOGIN`
- **登录**：`ipcMain.handle('ccs:auth:open-login')` 打开原生 `BrowserWindow`（1440×960 居中模态窗口），加载 `CCS_TENANT_LOGIN`。通过监听 `did-navigate` / `did-navigate-in-page` / `did-redirect-navigation` 事件自动检测登录成功
- **默认系统**：Electron 默认使用 `CCS_TENANT_URL` / `CCS_TENANT_LOGIN`（原生窗口无系统选择 UI，租户系统为默认）
- **优势**：原生窗口可读取任何 URL 而不受同源策略限制，登录成功自动检测，无需用户手动确认

#### Android（原生 Capacitor 插件）

- **网络检测**：通过 `ConnectivityManager.hasActiveNetwork()` 快速判断设备是否离线，离线直接放行
- **登录态检测**：在线时 `CcsAuthPlugin.check()` 创建 1×1 隐式 WebView 加载 `CCS_TENANT_LOGIN`，通过 `WebViewClient.onPageFinished` 获取最终 URL 判断登录态
- **登录**：`CcsAuthPlugin.openLogin()` 启动 `CcsLoginActivity`（全屏 WebView Activity），登录成功后通过 `startActivityForResult` 返回解析的租户 URL
- **默认系统**：Android 默认使用 `CCS_TENANT_URL` / `CCS_TENANT_LOGIN`（与 Electron 一致，原生端租户系统为默认）
- **架构文件**：
...
- **网络检测**：仅使用 `navigator.onLine` 判断设备是否离线（**不请求任何 CCS URL**），离线直接放行
- **登录方式**：Web 端**不再使用**内嵌登录 iframe / 系统选择下拉框 / 「进入系统」按钮，而是由 `decideWebLoginRedirect()` 决定将浏览器**整页重定向**（`window.location.replace`）到 CCS 系统自己的登录入口：
  - URL 携带 `?login=base` → 重定向到 `${CCS_BASE_URL}/ierp/?formId=ecc_index&redirect=<回跳地址>&canRedirect=true`
  - URL 携带 `?login=tenant` → 重定向到 `${CCS_TENANT_LOGIN}?redirect=<回跳地址>`（租户系统专用登录页）
  - 无 `?login=` 参数，但 ccs-framework 本身就部署在 `CCS_BASE_URL` / `CCS_TENANT_URL` 同源域名下（`isSameOriginAsCcs`）→ 同样自动重定向
  - 其他情况（无关域名上的裸 URL，如登录完成后的回跳）→ 不重定向，直接视为就绪
- **登录成功回跳**：CCS 登录页在已登录或登录成功后会跳回 `redirect` 地址；`canRedirect=true` 使已有有效会话时立即弹回，无需用户操作
- **防重定向循环**：`sessionStorage` 的 `ccs-web-login-redirect-attempted` 记录本次会话已尝试过重定向（及其目标系统 `base`/`tenant`），每个标签页会话最多重定向一次
- **租户参数**：从租户系统登录回跳后，页面 URL 预期携带 `tenant` 查询参数（缺省回退到 `CCS_Tenant_Gray`），据此将 `ccs-base-url-override` 升级为租户限定 URL
- **无持久化**：不记录任何登录历史状态，每次启动都是全新检测
...
2. URL 路径中**包含** `/ierp` 段（CCS 系统的特征路径）
3. URL **不等于** `CCS_LOGIN_URL` 本身（避免将登录页误判为已登录）

```typescript
// 判定逻辑（electron/main.ts 中的 JS 实现；Android 端 CcsAuthPlugin.java 中有对应的 Java 实现）
...
### 14.5 租户 URL 解析与会话级覆盖

登录成功后，系统从重定向 URL 中**自动提取带租户前缀的基础 URL**：

```
# 示例（以租户系统为例）
登录前: CCS_TENANT_URL = https://ccs.huaweicloud.com
...
**会话级存储**（不持久化，关闭标签页 / 应用后自动清除）：

| sessionStorage Key                 | 内容                                           | 用途                                                                                     |
| ---------------------------------- | ---------------------------------------------- | ---------------------------------------------------------------------------------------- |
| `ccs-base-url-override`            | 租户限定的基础 URL（如 `.../CCS_Tenant_Gray`） | 子模块 `IframePage.vue` 构建 iframe URL 时使用此覆盖值；**仅当前会话有效**，重启后清空   |
| `ccs-web-login-redirect-attempted` | `base` 或 `tenant`                             | Web 端整页重定向登录的防循环标记：记录本次会话已尝试重定向及目标系统；**仅当前会话有效** |

> **重要**：系统**不持久化任何登录历史状态**（无 `localStorage`、无 `ccs-auth-state`、无 `everLoggedIn` 标志）。每次应用启动都是全新的独立检测。`ccs-base-url-override` 使用 `sessionStorage`（而非 `localStorage`），唯一的用途是让同源的 `IframePage.vue` 在同一个浏览器会话中知道正确的租户 URL——它不代表"已登录"状态。
>
> 在 Electron / Android 平台上，所有路径（离线放行、登录成功、点击「进入系统」跳过登录）到达 `ready` 状态前都会确保写入平台正确的 base URL（默认 `CCS_TENANT_URL`），因此 `IframePage.vue` 不会错误地回退到 `CCS_BASE_URL`。
...
- **Electron**：主进程通过 `session.defaultSession.cookies.get({ url: baseUrl })` 按域名获取 Cookie，通过 IPC 返回给渲染进程
...
### 14.7 fetchProxy：全局同源认证请求代理

除原生平台的 `ccsAuthenticatedRequest` 外，框架还提供了**全平台通用**的同源请求代理 `fetchProxy`（`apps/ccs-framework/src/lib/fetch-proxy.ts`）：

- 认证门禁进入 `ready` 状态后，框架自动调用 `setupFetchProxy(baseUrl)`，创建一个指向 `${CCS 域名}/ierp/monorepo/proxy.html` 的**隐藏 iframe**，并在顶层 `window` 上暴露 `window.fetchProxy(url, init)` 函数
- 由于任何嵌套层级的 iframe（`ccs-module-common`、其门户卡片、以及更深层嵌入的业务页面）的 `window.top` 都指向框架的顶层窗口，因此**所有子页面**都可以通过 `window.top.fetchProxy(...)` 以同源身份调用 CCS 域名的受保护 API，无需各自搭建代理
- 代理内部通过 `postMessage` 请求-响应（带 30 秒超时）与 proxy.html 通信，返回标准 `Response` 对象，语义与 `window.fetch` 对齐
- 参考实现见 `apps/ccs-framework/public/ierp/monorepo/proxy.html`；本地可用 `pnpm ccs:ssl`（mock CCS 服务器，端口 63735）联调。生产环境需要真实的 CCS 域名部署一个等价的 `proxy.html`
...
> **离线优先设计**：只要设备处于离线状态（`navigator.onLine === false` / `net.isOnline() === false` / `hasActiveNetwork() === false`），应用**始终放行**，不请求任何 CCS URL，不查询任何登录状态。在线时则根据平台能力判断是否需要用户登录。
>
> **无持久化设计**：系统**不记录任何登录历史**——每次启动都是全新检测。关闭标签页 / 应用后，所有状态自动清除（`sessionStorage` 随会话结束自动清空）。这意味着即使是同一个浏览器，每次打开都是独立的登录流程。

---

## 15. AI 智能助手与 WebSkill 技能管理

### 15.1 概述

`ccs-framework` 内置了一套 **AI 智能助手系统**，包含前端对话界面、LLM 大模型集成、生成式 UI（OpenUI）渲染引擎、演示场景引擎、全链路运行观测和 WebSkill 技能管理器。所有相关代码位于 `apps/ccs-framework/src/skill/`，同样由 Google AI Studio 生成。

```
src/skill/
├── AIAssistantPanel.tsx       # AI 对话面板主组件
├── OpenUIRenderer.tsx         # 生成式 UI 渲染引擎（表单/KPI/看板/图表/仪表盘）
├── WebSkillManager.tsx        # WebSkill 技能管理器入口
├── types.ts                   # 类型定义（LLMConfig, OpenUISchema, ChatMessage 等）
├── context/
│   └── DataContext.tsx        # 全局数据中心（会话/追踪/演示数据/LLM 集成）
├── core/
│   ├── index.ts               # 核心模块导出
│   ├── skills.ts              # SKILL.md 解析、技能元数据与目录结构
│   └── errors.ts              # WebEntity 统一错误码体系
├── mock/                      # 演示静态数据（项目/需求/迭代/缺陷/测试套件/指标）及 mock/skills/ 技能定义（含远程/WebMCP 安装演示数据）
└── webskill-manager/          # 技能管理器子组件
    ├── SkillCatalogTab.tsx     # 技能目录（文件树浏览/编辑）
    ├── ImportExportTab.tsx     # 技能导入导出
    ├── RunObservationTab.tsx   # 运行观测日志
    ├── AuditEvaluationTab.tsx  # 审计评估
    ├── SkillOverview.tsx       # 技能概览
    ├── SkillPreviewModal.tsx   # 技能预览弹窗
    ├── SkillSummaryDashboard.tsx # 技能摘要仪表盘
    ├── FolderOverview.tsx      # 文件夹概览
    ├── FileTreeRenderer.tsx    # 文件树渲染器
    ├── UniversalFileEditor.tsx # 通用文件编辑器
    └── MOCK_SKILLS.ts         # 模拟技能数据
```

### 15.2 AI 智能助手对话框

`AIAssistantPanel` 是框架右侧的浮动对话面板，通过顶部工具栏的「AI 助手」按钮打开。

#### 15.2.1 核心功能

| 功能              | 说明                                                                               |
| ----------------- | ---------------------------------------------------------------------------------- |
| **LLM 配置**      | 可折叠的配置面板，支持自定义 API 密钥（密码框 + 显隐切换）、模型名称、接口端点     |
| **会话管理**      | 多会话支持：创建新会话、切换历史会话、删除会话；会话标题自动取首条用户消息前 12 字 |
| **流式输出**      | LLM 回复以打字机效果逐字呈现，伴有流式动画指示器                                   |
| **Markdown 渲染** | 使用 `react-markdown` + `remark-gfm` 渲染富文本，支持表格、代码块、引用等          |
| **快捷指令模板**  | 空会话时展示 3 个一键触发模板：分析迭代进展、缺陷严重程度分布、需求看板深度梳理    |
| **拖拽调整宽度**  | 桌面端支持左侧拖拽手柄调整面板宽度（320px ~ 1200px），带 iframe 事件穿透防护       |
| **移动端适配**    | 移动端自动切换全屏模式，使用底部滑入动画                                           |
| **配置持久化**    | LLM 配置和会话历史均持久化到 `localStorage`，刷新/重启后恢复                       |
...
sequenceDiagram
    ...
    participant UI as OpenUIRenderer

    User->>Panel: 输入自然语言问题
    Panel->>Context: sendChatMessage(text)
    Context->>Context: 追加 user 消息到当前会话
    Context->>Context: 创建 assistant 占位消息 (isStreaming=true)

    alt 命中演示场景（迭代/缺陷/看板）
        Context->>Context: 匹配本地 Mock 数据生成回复
        Context->>Context: 模拟流式逐字输出
        Context-->>Panel: 更新消息 text + uiSchema
        Panel->>UI: 渲染 OpenUI 组件
    else 未命中演示场景
        Context->>LLM: POST /chat/completions (stream=true)
        LLM-->>Context: SSE 流式返回 token
        Context-->>Panel: 逐 token 更新消息 text
        Context->>Context: 解析回复中的 JSON Schema
        Context-->>Panel: 更新消息 uiSchema
        Panel->>UI: 渲染 OpenUI 组件
    end
```

### 15.3 OpenUI：生成式 UI 渲染引擎

`OpenUIRenderer` 是 AI 助手回复中的**动态 UI 渲染引擎**。当 LLM 返回的 JSON 中包含 `__ui_schema__` 标记时，系统自动提取 Schema 并渲染为可交互的 UI 组件。

#### 15.3.1 支持的 UI 模板

| 模板类型     | `type` 值   | 渲染内容                                                      |
| ------------ | ----------- | ------------------------------------------------------------- |
| **表单**     | `form`      | 交互式表单（文本/下拉/多行/单选），支持提交动作触发后续分析   |
| **KPI 卡片** | `kpis`      | 2 列网格的指标卡片，带趋势箭头和颜色                          |
| **指标网格** | `metrics`   | 2 列 KPI 网格卡（与 `kpis` 类似但样式不同）                   |
| **看板**     | `kanban`    | 按状态分组的看板列（Todo/In Progress/Done），含优先级标签     |
| **表格**     | `table`     | 响应式数据表格，含列定义和行数据                              |
| **图表**     | `chart`     | 柱状图（bar）或环形图（donut），使用 SVG 渲染                 |
| **仪表盘**   | `dashboard` | 复合视图：KPI 指标 + 环形图 + 柱状图 + 多表格，一站式数据概览 |

#### 15.3.2 Schema 容错机制

LLM 并非总能严格遵循 JSON Schema 格式规范。`context/DataContext.tsx` 中的 `normalizeUiSchema()` 函数实现了多层容错：

- **顶层扁平化修复**：当 LLM 将 `columns`/`rows` 放在顶层而非嵌套在 `data` 中时，自动归位
- **列名兼容**：同时支持 `label` 和 `title` 作为列显示名
- **Dashboard 子组件打捞**：当 LLM 错误地使用 `children: [{ type: 'table', ... }]` 嵌套子组件时，自动提取为 `dashboardData.tables` 等标准结构
- **JSON 裸对象回退**：当 LLM 未使用 fenced code block（` ```json `）包裹时，从原文中扫描平衡的 `{...}` JSON 对象
- **缺失数组兜底**：Dashboard 必需的数组字段（`kpis`/`barPoints`/`donutPoints`/`tables`）自动初始化为空数组

#### 15.3.3 交互式表单（InteractiveForm）

表单支持三种自动提交动作：

| `formActionId`         | 触发行为                                                 |
| ---------------------- | -------------------------------------------------------- |
| `create-bug`           | 创建缺陷单并写入演示数据池，自动切换项目                 |
| `analyze-project`      | 提交后自动向 AI 发送 `分析迭代进度 {projectId}` 指令     |
| `analyze-bug-severity` | 提交后自动向 AI 发送 `分析缺陷严重程度 {projectId}` 指令 |
| `analyze-kanban`       | 提交后自动向 AI 发送 `分析项目需求 {projectId}` 指令     |

提交后表单区域替换为成功提示，并自动触发下一轮 AI 对话。

### 15.4 演示场景引擎

`DataContext` 内置了一套**离线演示数据引擎**，包含 4 个虚拟项目和完整的需求/迭代/缺陷数据集（`mock/` 目录）。当用户输入命中特定关键词模式时，系统无需调用 LLM API 即可直接生成带 OpenUI Schema 的结构化回复，方便离线演示和功能验证。

#### 15.4.1 支持的演示场景

| 触发关键词                                                               | 演示场景                     | 生成的 OpenUI                                        |
| ------------------------------------------------------------------------ | ---------------------------- | ---------------------------------------------------- |
| 包含 `sprint`/`迭代`/`进展`/`progress`                                   | 迭代进度分析（需先选项目）   | `form` → 自动触发 `dashboard`（KPI + 环形图 + 表格） |
| `分析迭代进度` 开头                                                      | 分析当前迭代进度             | `dashboard`（燃尽 SP、速率、状态分布环形图等）       |
| 包含 `缺陷`/`bug`/`incident` + `创建`/`新建`/`录入`/`提一`/`添加`/`报告` | 缺陷登记                     | `form`（项目选择 + 标题 + 严重级别 + 描述）          |
| `分析缺陷严重程度` 开头                                                  | 缺陷严重程度分布             | `dashboard`（KPI + 环形图 + 分级表格）               |
| 包含 `severity`/`严重程度`，或 `缺陷` + `分布`/`图表`/`chart`/`分类`     | 缺陷严重程度分布（先选项目） | `form` → 自动触发 `dashboard`                        |
| 包含 `看板`/`kanban` 或 `需求`+`梳理`/`状态`                             | 需求看板（需先选项目）       | `form` → 自动触发 `dashboard`                        |
| `分析项目需求` 开头                                                      | 需求优先级分布               | `dashboard`（KPI + 优先级环形图 + 分级表格）         |

#### 15.4.2 项目感知

所有演示场景均**感知当前选中的项目**。用户可在对话中通过 `PROJ-XXX` 格式指定目标项目，系统自动切换上下文。演示数据按 `projectId` 过滤，确保不同项目看到各自的需求/迭代/缺陷数据。

### 15.5 LLM 大模型集成

当用户输入**未命中**演示场景关键词时，系统将请求转发至配置的 LLM API。

#### 15.5.1 默认配置

| 配置项   | 默认值                        | 说明                             |
| -------- | ----------------------------- | -------------------------------- |
| API Key  | `VITE_API_KEY` 环境变量或内置 | 支持密码框安全输入，防止长度泄露 |
| Model    | `deepseek-v4-pro`             | 可通过 `VITE_MODEL_NAME` 覆盖    |
| Endpoint | `https://api.deepseek.com/`   | 可通过 `VITE_ENDPOINT` 覆盖      |

配置在 UI 中实时修改后立即生效，并通过 `localStorage` 持久化。

#### 15.5.2 流式调用

- 使用 `fetch` + `ReadableStream` 实现 SSE 流式读取
- 每个 token 到达时实时更新 UI（非全量替换），实现逐字输出效果
- 请求携带**完整对话历史**（当前会话的全部消息），确保 LLM 具有上下文记忆
- System Prompt 内置了 OpenUI Schema 生成规范，指导 LLM 按约定格式输出

#### 15.5.3 安全措施

- API Key 输入框默认 `type="password"`，支持显隐切换
- 未编辑时显示为 `••••••••••••••••` 掩码，防止屏幕窥探
- 配置存储在 `localStorage`，不上传至任何第三方

### 15.6 运行观测与链路追踪

系统内置了**全链路追踪（Tracing）系统**，每次 AI 对话都生成唯一的 `traceId`，记录技能从发现到执行完成的完整生命周期。
...
stateDiagram-v2
    [*] --> DISCOVERED : 用户输入触发
    DISCOVERED --> ACTIVATED : 技能匹配激活
    ACTIVATED --> EXECUTING : 开始执行
    EXECUTING --> AWAITING_USER : 需要用户输入(表单)
    AWAITING_USER --> EXECUTING : 用户提交表单
    EXECUTING --> COMPLETED : 执行完成
    COMPLETED --> EVALUATING : 结果评估
    EVALUATING --> [*]
```

> **说明**：`ERROR` 是预留的生命周期阶段，但当前实现不会发出该阶段——执行失败（如 LLM 调用异常）会以 `EVALUATING` 阶段、输出结果为 `ERROR` 的形式记录在追踪日志中。

#### 15.6.2 追踪指标

| 指标           | 说明                                                                                 |
| -------------- | ------------------------------------------------------------------------------------ |
| 已激活技能总数 | 累计被触发执行的技能数量                                                             |
| 执行成功次数   | 生命周期以 `EVALUATING` 结束的次数                                                   |
| 执行失败次数   | 预留指标（当前实现恒为 0：失败以 `EVALUATING` 阶段 + 输出 `ERROR` 记录，不单独计数） |
| 技能执行成功率 | 成功次数 / (成功+失败) × 100%                                                        |

#### 15.6.3 多轮对话追踪

当用户通过表单提交触发的场景（如选择项目后自动分析），第二次对话**复用同一个 `traceId`**，在追踪日志中形成连贯的调用链：

```
TraceID: a1b2c3d4
├── DISCOVERED  → analyze-project-iteration-progress
├── ACTIVATED   → 分析当前冲刺迭代进度数据
├── EXECUTING   → 生成包含所有项目的选择表单
├── AWAITING_USER → 用户在表单里选择项目
├── EXECUTING   → 调用获取迭代数据
├── EXECUTING   → 结合图表 JSON 渲染模块
├── COMPLETED   → 当前项目产品需求规划概览指标
└── EVALUATING  → 成功率 100% → ACTIVATE
```

所有追踪日志可通过 **WebSkill 管理器 → 运行观测** 标签页查看，支持按 TraceID/技能名称/生命周期阶段搜索和分页浏览。

### 15.7 WebSkill 智能技能管理器

`WebSkillManager` 是技能的全生命周期管理面板，通过 AI 助手对话框顶部的魔方块图标进入（点击后关闭对话框并导航到技能管理页）。包含 4 个功能标签页：

#### 15.7.1 技能目录（Catalog）

- **文件树浏览器**：左侧显示技能文件夹树，支持展开/折叠
- **节点管理**：新建技能、新建文件/文件夹、重命名、删除（含确认弹窗）
- **文件编辑**：内嵌 `UniversalFileEditor` 通用编辑器，支持代码高亮和格式化
- **技能概览**：右侧展示选中技能的元数据（名称、描述、版本、标签）
- **SKILL.md 解析**：使用 `parseSkillMarkdown()` 解析 YAML frontmatter

#### 15.7.2 导入导出（Import/Export）

- **安装远程技能**：从外部 HTTP/HTTPS 源安装 WebSkill
- **安装 WebMCP 技能**：基于 MCP 协议动态加载 Web 页面技能
- **上传技能包**：支持 `.zip` / `.tar` 压缩包上传
- **导出技能包**：将当前技能打包为通用分发格式
- **前置校验**：安装前强制校验 WebSkill 配置结构

#### 15.7.3 运行观测（Run Observation）

- 汇总统计栏：已激活技能数、成功/失败次数、成功率
- 实时日志表格：TraceID、生命周期阶段（带颜色标签）、技能名称、输入参数、输出结果、执行耗时、记录时间
- 搜索过滤：支持按 TraceID / 技能名称 / 阶段搜索
- 分页浏览：每页 10 条，支持翻页

#### 15.7.4 审计评估（Audit Evaluation）

- **技能评分策略**：设定高/低评分阈值
- **高评分策略**：技能保持激活状态，允许更新升级版本
- **低评分策略**：触发技能回滚或卸载提示，避免低质量技能持续使用
- **操作按钮**：查看、更新、回滚、卸载（根据评分动态启用/禁用）

### 15.8 技能元数据核心库

`core/` 目录提供了技能系统的底层能力，独立于 React UI 运行。

#### 15.8.1 SKILL.md 解析

`parseSkillMarkdown(raw, location)` 函数解析标准 `SKILL.md` 文件：

```typescript
// SKILL.md 格式要求
---
name: analyze-project-iteration
version: 1.0.0
description: 分析项目迭代进度并生成可视化报告
tags: [agile, sprint, chart]
author: CCPS Team
---
...
- YAML frontmatter 必须以 `---` 开头和闭合
- `name` 和 `description` 为必填字段，缺失时抛出 `SKILL_INVALID_METADATA` 错误
- 返回 `SkillDocument` 对象，包含 metadata、body、raw、location

#### 15.8.2 WebEntity 错误码体系

```typescript
type WebEntityErrorCode =
  | 'FS_NOT_FOUND'           // 文件未找到
  | 'FS_PERMISSION_DENIED'   // 文件权限拒绝
  | 'SKILL_INVALID_METADATA' // 技能元数据无效
  | 'SKILL_DUPLICATE_NAME'   // 技能名称重复
  | 'SKILL_NOT_FOUND'        // 技能未找到
  | 'TOOL_EXECUTION_FAILED'  // 工具执行失败
  | 'MCP_ENDPOINT_UNAVAILABLE' // MCP 端点不可用
  | 'UI_REQUIRED'            // 需要 UI 交互
...
所有业务层错误通过 `WebEntityError` 类统一抛出，携带稳定 `code`、可选 `details` 和底层 `cause`，便于 HTTP/API 适配器将内部失败映射为可诊断的结构化响应。
...
interface SkillCatalogEntry {
  ...
  hasReferences: boolean; // 是否包含引用
  hasAssets: boolean; // 是否包含静态资源
  source: 'local' | 'mcp' | 'generated' | 'installed'; // 来源
}
```
