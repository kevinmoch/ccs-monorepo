...
| 环境变量           | 说明                                                        | 必填              |
| ------------------ | ----------------------------------------------------------- | ----------------- |
| `CCS_BASE_URL`     | CCS ERP 主系统的基础 URL（Web 端默认使用）                  | 生产环境必填      |
| `CCS_LOGIN_URL`    | CCS ERP 主系统的登录页 URL（Web 端默认使用）                | 生产环境必填      |
| `CCS_TENANT_URL`   | CCS ERP 租户系统的基础 URL（Electron / Android 默认使用）   | 生产环境必填      |
| `CCS_TENANT_LOGIN` | CCS ERP 租户系统的登录页 URL（Electron / Android 默认使用） | 生产环境必填      |
...
| 跨域 Web 端需手动选择系统  | 跨域场景无法自动检测登录，需通过下拉框选择系统并（选择租户系统时）填写租户名后点击「进入系统」      |
...
stateDiagram-v2
    ...
    login_required --> ready : 登录成功（原生自动检测）或点击「进入系统」
...
| `login-required` | 在线但未登录，需要用户完成登录（或手动跳过）       | 全屏登录 iframe / 原生登录窗口（见 [14.3](#143-各平台实现差异)） |
...
- **登录**：`ipcMain.handle('ccs:auth:open-login')` 打开原生 `BrowserWindow`（全屏），加载 `CCS_TENANT_LOGIN`。通过监听 `did-navigate` / `did-navigate-in-page` / `did-redirect-navigation` 事件自动检测登录成功
...
- **登录态**：Web 端**不验证**实时登录状态（跨域限制），在线时总是进入 `login-required` 流程
- **同源场景**（`ccs-framework` 与 `CCS_LOGIN_URL` 同源）：可直接读取 iframe 的 `contentWindow.location.href`，自动检测登录成功，同时顶部有「进入系统」兜底按钮
- **跨源场景**：无法读取 iframe URL，无法自动检测登录。顶部显示系统选择下拉框（`CCS_BASE_URL` / `CCS_TENANT_URL`），选择 `CCS_TENANT_URL` 时需填写租户名，点击「进入系统」确认
...
LLM 并非总能严格遵循 JSON Schema 格式规范。`normalizeUiSchema()` 函数实现了多层容错：
...
| 包含 `缺陷`/`bug` + `创建`/`新建`/`录入`     | 缺陷登记                   | `form`（项目选择 + 标题 + 严重级别 + 描述）          |
