...
// 2) 发起一次真实请求（需已打开目标子网站页面）
const r = await window.ccsExtFetch('/ierp/kapi/app/MaterialTemplate/call', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
...
| 报"未找到已打开的目标子网站页面" | 先在外壳中打开对应子网站页面（如待办任务），再发起请求                             |
...
- 设计方案：`CCS_Fetch_代理扩展_f5b922fc.md`（仓库根目录）
