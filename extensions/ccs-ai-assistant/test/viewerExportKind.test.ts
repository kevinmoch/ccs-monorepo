...
describe('内置技能的判型', () => {
  ...
  /** 编排型技能没有脚本（FR-19.13），下面那条回放无从取据——但它仍须在判型表里有一行 */
  const publishing = skills.filter((name) => existsSync(resolve(BUILTIN_DIR, name, 'scripts/publish.js')));

  it('每个内置技能都在判型表里有一行', () => {
    expect([...skills].sort()).toEqual(Object.keys(BUILTIN_EXPORT_KIND).sort());
  });

  it.each(publishing)('%s 的判型与它 publish.js 落到根节点上的标记一致', (name) => {
