 ...
 *
 * 与 `pageMcpBridge.ts` 同一条规矩：**不依赖任何 SDK 类型**。
 * MAIN 侧那一端由 `probe.js` 承载，它是 `document_start` 注入到每一帧的脚本，
 * 把 `@webskill/browser` 的类型图拖进去，等于让每次导航都多下载一份用不上的代码。
 *
 * 代价是这里有两份「同一个事实」的副本——只读方法名与 `OfficeType` 指纹。
 * 副本不靠自觉同步：仓库根的 `test/webOfficeMirrors.test.ts` 读两侧声明逐条比对。
 */
